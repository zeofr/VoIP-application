...
def select_device(devices, device_type="input"):
    ...
    # Remove duplicates
    unique_devices = list({(index, name, channels, tuple(rates)) for index, name, channels, rates in devices})
    unique_devices.sort(key=lambda x: x[1])  # Optional: sort by device name

    for i, (index, name, channels, rates) in enumerate(unique_devices):
        print(f"{i+1}. {name} (Channels: {channels}, Rates: {rates})")

    while True:
        try:
            choice = int(input(f"Select {device_type} device (1-{len(unique_devices)}): ")) - 1
            if 0 <= choice < len(unique_devices):
                selected = unique_devices[choice]
